  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (mp3Files.length === 0) {
        toast.error('Nessun file MP3 selezionato');
        return;
      }

      await addTracksFromFiles(mp3Files);
      toast.success(`${mp3Files.length} brano/i aggiunto/i!`);
    } catch (error) {
    ...
    <div className="min-h-screen pb-36 safe-top">
      ...
      <main className="px-4 space-y-6">
        ...
        <section>
          ...
          <div className="bg-card rounded-2xl overflow-hidden p-4 space-y-3">
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={() => fileInputRef.current?.click()}
              disabled={isLoadingFiles}
              className="w-full flex items-center justify-center gap-3 py-4 rounded-xl bg-primary text-primary-foreground font-semibold"
            >
              {isLoadingFiles ? (
                <>
            ...
            <p className="text-xs text-muted-foreground text-center">
              Seleziona uno o più file MP3 dal tuo dispositivo
            </p>
          </div>
        </section>
            ...
            <div className="bg-card rounded-2xl overflow-hidden">
              <div className="p-4">
                ...
                <motion.button
                  ...
                  disabled={isScanning}
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold bg-secondary text-secondary-foreground"
                >
                  <RefreshCw size={18} className={isScanning ? 'animate-spin' : ''} />
                  {isScanning ? 'Scansione...' : 'Scansiona cartelle'}
                </motion.button>
              </div>
            </div>
          </section>
